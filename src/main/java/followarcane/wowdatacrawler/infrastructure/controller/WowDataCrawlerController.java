...
public class WowDataCrawlerController {
    ...
    @PostMapping("/crawl")
    @ResponseBody
    public ResponseEntity<List<CharacterInfoDTO>> crawl() {
        List<CharacterInfo> characterInfoList = wowDataCrawlerService.crawlCharacterInfoFromWeb("https://www.wowprogress.com/gearscore/?lfg=1&sortby=ts");
        List<CharacterInfoDTO> responseDtoList = characterInfoList.stream()
